...
describe('AdminService', () => {
  beforeAll(async () => {
    ...
    await env.DB.exec(
      `INSERT INTO events (name, slug, date, time, location, organizer, organizer_email, created_by) VALUES ('Past Event', 'past-event', '${pastDate}', '10:00', 'Stockholm', 'Test', 'test@consid.se', 'test@consid.se');`,
    );
    await env.DB.exec(
      `INSERT INTO events (name, slug, date, time, location, organizer, organizer_email, created_by) VALUES ('Future Event', 'future-event', '${futureDate}', '18:00', 'Göteborg', 'Test', 'test@consid.se', 'test@consid.se');`,
    );

    // Add participants
    await env.DB.exec(
      `INSERT INTO participants (event_id, name, email, category, cancellation_token) VALUES (1, 'P1', 'p1@test.se', 'internal', 'token-admin-p1');`,
    );
    await env.DB.exec(
      `INSERT INTO participants (event_id, name, email, category, cancellation_token) VALUES (1, 'P2', 'p2@test.se', 'public_sector', 'token-admin-p2');`,
    );
    await env.DB.exec(
      `INSERT INTO participants (event_id, name, email, category, cancellation_token) VALUES (2, 'P3', 'p3@test.se', 'internal', 'token-admin-p3');`,
    );

    // Add a mailing
    await env.DB.exec(
      `INSERT INTO mailings (event_id, subject, body, status) VALUES (1, 'Testutskick', 'Hej alla', 'sent');`,
    );
  });
  ...
  it('admin user canView/canEdit any event without explicit permission', async () => {
    const user = await createUser(
      env.DB,
      'admin-view@consid.se',
      'Admin View',
      'token-admin-view-test',
    );
    await env.DB.exec(`UPDATE users SET is_admin = 1 WHERE id = ${user.id};`);
