...
export async function listMailings(db: D1Database, eventId: number): Promise<Mailing[]> {
  const result = await db
    .prepare('SELECT * FROM mailings WHERE event_id = ? ORDER BY created_at DESC')
    .bind(eventId)
    .all<Mailing>();
...
export async function getMailingById(db: D1Database, id: number): Promise<Mailing | null> {
  const result = await db.prepare('SELECT * FROM mailings WHERE id = ?').bind(id).first<Mailing>();
...
export async function createMailing(
  ...
  const mailing = await db.prepare('SELECT * FROM mailings WHERE id = ?').bind(id).first<Mailing>();
...
export async function updateMailing(
  ...
  await db
    .prepare(`UPDATE mailings SET ${fields.join(', ')} WHERE id = ? AND status = 'draft'`)
    .bind(...values)
    .run();
...
export async function markMailingSent(db: D1Database, id: number): Promise<Mailing | null> {
  const now = new Date().toISOString();
...
export async function updateParticipantStatus(
  ...
  const result = await db
    .prepare('UPDATE participants SET status = ?, updated_at = ? WHERE cancellation_token = ?')
