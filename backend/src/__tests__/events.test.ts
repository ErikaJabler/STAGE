...
describe('Events CRUD API', () => {
  ...
  it('GET /api/events/:id returns a created event', async () => {
    ...
    const createRes = await request(
      'POST',
      '/api/events',
      makeEvent({ name: 'Get Single', slug: 'get-single' }),
    );
  ...
  it('GET /api/events/:id returns 403 for event without permission', async () => {
    ...
    await env.DB.exec(
      `INSERT OR IGNORE INTO users (email, name, token) VALUES ('other@consid.se', 'Other', 'other-token')`,
    );
  ...
  it('PUT /api/events/:id updates an event', async () => {
    ...
    const createRes = await request(
      'POST',
      '/api/events',
      makeEvent({ name: 'To Update', slug: 'to-update' }),
    );
    const created = (await createRes.json()) as { id: number };
  ...
  it('DELETE /api/events/:id soft-deletes an event', async () => {
    ...
    const createRes = await request(
      'POST',
      '/api/events',
      makeEvent({ name: 'To Delete', slug: 'to-delete' }),
    );
    const created = (await createRes.json()) as { id: number };
  ...
  it('GET /api/events includes participant_count', async () => {
    const createRes = await request(
      'POST',
      '/api/events',
      makeEvent({ name: 'With Count', slug: 'with-count' }),
    );
