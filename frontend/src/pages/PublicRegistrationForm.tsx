...
export function PublicRegistrationForm({ event, slug, onRegistered }: PublicRegistrationFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} style={s.form}>
      ...
      <FormField
        label="Allergier / kostpreferenser"
        value={form.dietary_notes}
        onChange={(v) => set('dietary_notes', v)}
...
export function RegistrationConfirmation({
  ...
  return (
    <div style={s.confirmationBox}>
      <div style={s.confirmIcon}>{regResult.waitlisted ? '⏳' : '🎉'}</div>
...
function downloadICS(event: Event) {
  ...
  const lines = [
    ...
    'END:VTIMEZONE',
    'BEGIN:VEVENT',
    `UID:${event.date}-${event.slug}@stage`,
    `DTSTAMP:${new Date()
      .toISOString()
      .replace(/[-:]/g, '')
      .replace(/\.\d{3}/, '')}`,
